...
def test_Hamiltonian():
    ...
    z = ham.partition_function(1,conf_sys)
    
    assert ham.compute_energy(conf) == 24.8
    assert ham.compute_energy(conf,True) == '-8J/k + 8mu/k'
    assert (round(ham.compute_average_energy(1,conf_sys,z),3) == -3.991)
    assert (round(ham.compute_average_mag(1,conf_sys,z),3) == -0.003)
    assert (round(ham.compute_heat_capacity(1,conf_sys,z),3) == 0.053)
    assert (round(ham.compute_mag_susceptibility(1,conf_sys,z),3) == 0.006)
